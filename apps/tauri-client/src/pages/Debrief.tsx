  ...
  return (
    <div class="debrief">
      ...
      <section>
        ...
        <p class="help-text">
          Mark sections of the track where engineer messages should be suppressed.
          Zones defined by lap distance percentage (0–1).
        </p>
        {zones.length === 0 && <p class="placeholder">No blackout zones configured.</p>}
        {zones.map((z) => (
          <div key={z.id} class="zone-row">
            <span>{z.label}</span>
            <span>{(z.startPct * 100).toFixed(0)}% – {(z.endPct * 100).toFixed(0)}%</span>
            <button onClick={() => setZones((prev) => prev.filter((z2) => z2.id !== z.id))}>
              Remove
            </button>
          </div>
        ))}
        <button onClick={() => setZones((prev) => [...prev, { id: crypto.randomUUID(), startPct: 0.1, endPct: 0.2, label: "New zone" }])}>
          + Add Zone
        </button>
      </section>
    </div>
