...
// Wire shape per data-model.md DebugSnapshot: tri-states arrive as
// boolean | null — the display-string mapping happens HERE, never on the wire.
interface DebugSnapshot {
  ...
  whisperModelLoaded: boolean | null;
}
...
function TriBadge({
  ...
  label: string;
  value: boolean | null;
  mapping: [string, string, string]; // [null-display, true-display, false-display]
}) {
  const display = value === null ? mapping[0] : value ? mapping[1] : mapping[2];
  const color = value === null ? '#9ca3af' : value ? '#22c55e' : '#ef4444';
  return (
...
/** M10 US5 (T029): the fixed-variable live snapshot fed by get_debug_snapshot
 *  (initial load) + the 1Hz telemetry:debug-snapshot event. */
function LiveSnapshotSection() {
  const [snapshot, setSnapshot] = useState<DebugSnapshot | null>(null);
  const lagWarnRef = useRef<LagWarningState>(initialLagWarningState);
  const [lagWarningVisible, setLagWarningVisible] = useState(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        unlisteners.push(
          await listen<DebugSnapshot>('telemetry:debug-snapshot', (e) => {
            if (cancelled) return;
            setSnapshot(e.payload);
            // The reducer owns the SC-005 first-snapshot guard + 5s dismissal
            // hysteresis; only EVENTS feed it (the initial invoke never warns).
            lagWarnRef.current = lagWarningReducer(
              lagWarnRef.current,
              e.payload.redisStreamLagMs,
              Date.now(),
            );
            setLagWarningVisible(lagWarnRef.current.visible);
          }),
        );
      } catch (err) {
        console.error('debug snapshot listener failed:', err);
      }
    })();
    return () => {
      cancelled = true;
  ...
  return (
    <section style={{ marginBottom: '1.5rem' }}>
      <h2 style={{ margin: '0 0 0.5rem' }}>Live Snapshot</h2>
      {!snapshot ? (
        ...
        <>
          <p style={{ margin: '0 0 0.5rem' }}>
            {snapshot.sessionActive ? (
              <span style={{ color: '#22c55e', fontWeight: 600 }}>
                Active{snapshot.sessionId ? ` — ${snapshot.sessionId}` : ''}
              </span>
            ) : (
          ...
          <table style={{ borderCollapse: 'collapse', marginBottom: '0.5rem' }}>
            <tbody>
              {(
                [
                  ['Fuel remaining', fmt(snapshot.fuelRemaining), 'L'],
                  ['Current lap', fmt(snapshot.currentLap, 0), ''],
                  ['Track position', fmt(snapshot.trackPosition, 3), ''],
                  ['Lap time delta', fmt(snapshot.lapTimeDelta), 's'],
                ] as const
          ...
          <p style={{ margin: '0 0 0.25rem' }}>
            <TriBadge
              label="Redis"
              value={snapshot.redisConnected}
              mapping={['Checking…', 'Connected', 'Disconnected']}
            />
            {/* hubConnected === null → neutral/checking (grey), per the
                data-model B3 mapping — "unknown" never crosses the wire. */}
            <TriBadge
              label="Hub"
              value={snapshot.hubConnected}
              mapping={['Checking…', 'Connected', 'Disconnected']}
            />
            <TriBadge
              label="Whisper"
              value={snapshot.whisperModelLoaded}
              mapping={['Loading…', 'Ready', 'Load failed']}
            />
          </p>
          <p style={{ margin: 0 }}>
            <span style={{ color: '#888' }}>Stream lag: </span>
            <span
              style={{
                color:
                  snapshot.redisStreamLagMs !== null && snapshot.redisStreamLagMs > 500
                    ? '#ef4444'
                    : undefined,
              }}
            >
              {snapshot.redisStreamLagMs === null ? '—' : `${snapshot.redisStreamLagMs} ms`}
            </span>
            {lagWarningVisible && (
              <span
                role="alert"
                style={{ color: '#ef4444', fontWeight: 600, marginLeft: '0.5rem' }}
              >
                ⚠ Stream lag over 500 ms
              </span>
            )}
          </p>
        </>
...
export function DebugPanel() {
  ...
  const [log, setLog] = useState<LogEntry[]>([{ ts: nowHMS(), msg: 'Debug panel mounted' }]);
