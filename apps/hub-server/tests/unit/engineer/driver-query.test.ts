...
function makeEngineer(run: SynthDeps['runLlm']): { engineer: RacingEngineerService; conn: FakeRedis } {
...
describe('driver-query flow — engineer:query → voice:audio (US1)', () => {
  it('transcribed query is synthesized and published as Tier 3 clips', async () => {
    const okRun: SynthDeps['runLlm'] = async (_c, _m, _t, opts) => {
      ...
      return { status: 'ok', text: 'Box this lap. Fuel is tight.', toolsCalled: ['get_fuel_status'], latencyMs: 10 } as LlmResult;
    };
    const { engineer, conn } = makeEngineer(okRun);
    active = engineer;
    await engineer.start();

    conn.deliver('engineer:query', JSON.stringify({ queryId: 'q1', transcript: 'do we pit this lap?', sessionId: 's1', capturedAtMs: 1 }));
  ...
  it('ignores an empty transcript (no synthesis, no publish)', async () => {
    const { engineer, conn } = makeEngineer(async () => ({ status: 'ok', text: '', toolsCalled: [], latencyMs: 1 }));
    active = engineer;
    await engineer.start();

    conn.deliver('engineer:query', JSON.stringify({ queryId: 'q1', transcript: '   ', sessionId: 's1', capturedAtMs: 1 }));
  ...
  it('drops queries beyond queueDepthCap while one is in flight (Q4)', async () => {
    ...
    try {
      ...
      for (let i = 0; i < 4; i++) {
        conn.deliver('engineer:query', JSON.stringify({ queryId: `q${i}`, transcript: 'do we pit?', sessionId: 's1', capturedAtMs: i }));
      }
