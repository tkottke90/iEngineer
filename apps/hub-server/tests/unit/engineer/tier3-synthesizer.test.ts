...
function deps(llmResult: LlmResult, streamText: string[]): { deps: Partial<SynthDeps>; order: string[]; finalized: { outcome?: string; response?: string | null } } {
  const order: string[] = [];
...
describe('Tier3Synthesizer — happy path (T029/T030)', () => {
  it('audits before acting, enqueues per-sentence Tier 3 clips, finalizes synthesized', async () => {
    ...
    const { deps: d, order, finalized } = deps(
      { status: 'ok', text: 'Box this lap. Fuel is tight.', toolsCalled: ['get_fuel_status'], latencyMs: 12 },
      ['Box this lap. ', 'Fuel is tight.'],
    );
    const synth = new Tier3Synthesizer(raceState, memory, tools, queue, CONFIG, d);

    await synth.synthesize({ type: 'driver-query', triggerSource: 'q1', userText: 'do we pit?', personality: P3 });
...
describe('Tier3Synthesizer — degradation + suppression', () => {
  it('LLM unreachable: finalizes skipped and enqueues a canned line for a driver-query (FR-023)', async () => {
    ...
    const synth = new Tier3Synthesizer(raceState, new SessionMemoryStore('s1'), createTools({ getFuelModel: () => null, getTireModel: () => null }), queue, CONFIG, d);
  ...
  it('Energy=1 suppresses proactive commentary (no audit, no enqueue)', async () => {
    ...
    await synth.synthesize({ type: 'post-sector', triggerSource: 'hero:lap_complete', userText: 'comment on the lap', personality: { ...P3, energy: 1 } });
  ...
  it('Energy=1 still answers a direct driver-query', async () => {
    const queue = new PriorityMessageQueue();
    const { deps: d, order } = deps({ status: 'ok', text: 'Yes, box now.', toolsCalled: [], latencyMs: 5 }, ['Yes, box now. ']);
    const synth = new Tier3Synthesizer(raceState, new SessionMemoryStore('s1'), createTools({ getFuelModel: () => null, getTireModel: () => null }), queue, CONFIG, d);

    await synth.synthesize({ type: 'driver-query', triggerSource: 'q1', userText: 'should I pit?', personality: { ...P3, energy: 1 } });
