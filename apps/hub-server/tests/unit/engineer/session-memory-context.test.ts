...
import type { RaceState, EngineerConfig, PersonalityConfig, FuelModel } from '@iracing-engineer/types';
...
const CONFIG = { deferenceThreshold: 2, llm: { tokenBudget: 6000, timeoutMs: 1000, maxResponseTokens: 300, baseUrl: 'x', model: 'm', provider: 'openai-compatible' } } as unknown as EngineerConfig;
const P3: PersonalityConfig = { openness: 3, warmth: 3, energy: 3, conscientiousness: 3, assertiveness: 3 };
const fuelModel = { lapsRemaining: 3, burnRatePerLap: 2.6, fuelRemaining: 8, fuelDeficit: 1.2, confidenceLevel: 'high', dataSource: 'measured', lapsSinceCalibration: 4 } as unknown as FuelModel;

function raceState(): RaceState {
  return { session: { sessionId: 's1', sessionPhase: 'Race', lapsRemaining: 20, flags: 0 }, field: {}, hero: { position: 4, lapDistPct: 0.5, fuelLevel: 8, tireCompound: 'soft', lastLapTime: 92, onPitRoad: false, fuelUsePerHour: 40, lapDeltaToBest: 0.3, gapToLeader: 12, waterTemp: 88, oilTemp: 95 }, signals: { pitWindowOpen: true, safeWindowOpen: true } } as unknown as RaceState;
}
...
function makeSynth(mem: SessionMemoryStore): { synth: Tier3Synthesizer; userMsg: () => string } {
  ...
  const synth = new Tier3Synthesizer(raceState, mem, createTools({ getFuelModel: () => fuelModel, getTireModel: () => null }), new PriorityMessageQueue(), CONFIG, deps);
...
describe('session memory in reasoning (US6, FR-011/018)', () => {
  it('an earlier recommendation + override outcome is surfaced in the reasoning context', async () => {
    ...
    await synth.synthesize({ type: 'driver-query', triggerSource: 'q1', userText: 'and now?', personality: P3 });
  ...
  it('populates the fuel calibration from the M3 model into the context', async () => {
    const mem = new SessionMemoryStore('s1');
    const { synth, userMsg } = makeSynth(mem);
    await synth.synthesize({ type: 'driver-query', triggerSource: 'q1', userText: 'fuel?', personality: P3 });
